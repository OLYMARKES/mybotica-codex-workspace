...
ASSETS = ROOT / "assets"
OUT = ROOT / "storyboards" / "chat-overload-v4"
OUT.mkdir(parents=True, exist_ok=True)
GEN_OUT = ROOT / "generation" / "chat-overload-v4"
...
def frame_9():
    ...
    d = ImageDraw.Draw(im)
    paste_logo(im, (100, 470, 980, 820))
    d.line((180, 900, 900, 900), fill=RED, width=12)
    d.multiline_text((540, 1060), "Клиенты пишут.\nMyBotica отвечает.\nТы пьёшь кофе.", font=font(62, True), fill=CREAM, anchor="ma", align="center", spacing=22)
    return im
...
clean_frames[7] = Image.new("RGBA", (W, H), PAPER)
c8d = ImageDraw.Draw(clean_frames[7])
c8d.ellipse((-160, 340, 1240, 1740), fill=GREEN)
paste_mark(clean_frames[7], (225, 600, 855, 1260), white=True)
c8d.arc((-210, 290, 1290, 1790), 205, 500, fill=RED, width=20)

for i, frame in enumerate(clean_frames, 1):
    frame.save(GEN_OUT / f"frame-{i:02d}.png")
...
sd.text((sheet.width // 2, 120), "раскадровка v4 · живые вопросы → спасение → кофе → слоган", font=font(25), fill=RED, anchor="ma")
...
sheet.save(ROOT / "storyboard-chat-overload-v4.jpg", quality=95)
print(ROOT / "storyboard-chat-overload-v4.jpg")
