...
ASSETS = ROOT / "assets"
OUT = ROOT / "storyboards" / "chat-overload-v2"
...
def base(index, timecode, title, green=False):
    ...
    d.text((138, 85), f"{index}/8", font=font(33, True), fill=WHITE, anchor="mm")
...
def frame_5():
    im, d = base(5, "0:08–0:10", "ПАУЗА")
    paste_pose(im, 7, (210, 690, 870, 1430))
    bubble(d, (70, 500), "...", muted=True, width=320)
    bubble(d, (680, 560), "...", muted=True, width=300)
    bubble(d, (80, 1370), "...", muted=True, width=300)
    d.text((540, 1640), "Сообщения замирают. Он выдыхает.", font=font(40), fill=MUTED, anchor="mm")
    return im


def frame_6():
    im, d = base(6, "0:10–0:12", "ЗНАК ОТДЕЛЯЕТСЯ")
    paste_pose(im, 12, (330, 940, 930, 1570))
    paste_mark(im, (250, 380, 830, 900))
    for y, radius in [(980, 8), (910, 12), (830, 16)]:
        d.ellipse((530 - radius, y - radius, 530 + radius, y + radius), fill=RED)
    d.text((540, 1680), "Поднимается графический дубль головы", font=font(38), fill=GREEN, anchor="mm")
    return im


def frame_7():
    im, d = base(7, "0:12–0:14", "ЗНАК ЗАНИМАЕТ ЭКРАН")
    ...
    d.text((540, 1570), "Зелёный цвет растёт из самого знака", font=font(38), fill=CREAM, anchor="mm")
    return im


def frame_8():
    im, d = base(8, "0:14–0:18", "MYBOTICA", green=True)
    paste_logo(im, (100, 510, 980, 850))
    d.line((180, 930, 900, 930), fill=RED, width=12)
    d.multiline_text((540, 1120), "Скоро MyBotica будет\nотвечать за вас", font=font(58, True), fill=CREAM, anchor="ma", align="center", spacing=18)
...
sd.text((sheet.width // 2, 120), "раскадровка v2 · молочный мир → знак → зелёный бренд-финал", font=font(29), fill=RED, anchor="ma")
...
sheet.save(ROOT / "storyboard-chat-overload-v2.jpg", quality=95)
print(ROOT / "storyboard-chat-overload-v2.jpg")
